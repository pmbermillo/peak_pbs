...
import { AddLocation, GetLocation, DeleteLocation } from '../../Api/Module/SettingsApi';
...
const LocationManagement = () => {
	...
	const fetchData = async () => {
		try {
			...
			if (response && response.data[0]) {
				setDataLocation(response.data[0]);
			}
		...
		try {
			MySwal.fire({
				...
				if (result.isConfirmed) {
					try {
						const response = await DeleteLocation({id}); // your API call
	...
	return (
		<div>
			...
			<Table title="Category" columns={columns} data={dataLocation} />
		</div>
	)
}
