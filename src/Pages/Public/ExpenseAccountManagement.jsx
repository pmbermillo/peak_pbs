...
import { AddExpenseAccount, GetExpenseAccount, DeleteExpenseAccount } from '../../Api/Module/SettingsApi';

const ExpenseAccountManagement = () => {
    ...
    const columns = [
        { header: "Expense Account Name", accessor: "expense_account" },
    ...
    const fetchData = async () => {
        try {
            const response = await GetExpenseAccount();
            if (response && response.data[0]) {
                setDataExpense(response.data[0]);
            }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await AddExpenseAccount(formData);
            console.log(response);
            ...
            toast.error(
                error.response?.data?.errors?.expense_account?.[0] ??
        ...
        try {
            MySwal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await DeleteExpenseAccount({id}); // your API call
            ...
            toast.error(
                error.response?.data?.errors?.expense_account?.[0] ??
                error.response?.data?.error ??
                'Failed to delete expense account.'
            );
        }
    }	
    ...
    return (
        <div>
            ...
            <Table title="Project" columns={columns} data={dataExpense} />
        </div>
    )
}
